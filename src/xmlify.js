...
function renderMustache(str, data, envdata = null) {
...
function renderMustacheStr(str, data, envdata = null) {
    //console.log("render=> %s", str)
    let cmd = "";
...
function traversalNode(node, prefix, data, envStack, xml) {
    ...
    if ("js-for" in attrs) {
        const forValue = renderMustache(attrs["js-for"], data, envData);
...
function xmlify(data) {
    ...
    `;
    traversalNode(doc.documentElement, "", data.data, envStack, (str) => {
        xml += str;
    });
    return xml;
}
module.exports = xmlify;
//# sourceMappingURL=xmlify.js.map